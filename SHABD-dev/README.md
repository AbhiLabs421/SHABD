...
## Enterprise stack — identity & cache with **no** Docker, no dependencies

Regulated / air-gapped shops usually install Keycloak (a Java + container stack)
and Redis just to get login and shared cache. SHABD ships pure-Python built-ins
for both — each with an external alternative you pick in one `config.yaml`:

| Need | Built-in (pure Python, no deps) | External option |
|------|---------------------------------|-----------------|
| Identity / SSO | **Praman** — OAuth2/OIDC: grants, JWKS (RS256), TOTP MFA, lockout | your Keycloak |
| Cache / coordination | **Smriti** — RESP server, TTL, distributed rate-limit | your Redis |
| Persistence | JSONL / SQLite (encrypted optional) | Postgres |
| Signing secret | env / file | HSM |

```yaml
# config.yaml — one file decides the whole posture
identity: { provider: builtin }     # or: keycloak (external URL)
cache:    { provider: builtin }     # or: smriti (own server) | redis
```

```bash
# launch the built-in identity + cache servers — no image to pull
python -m shabd_config --config config.yaml
```

**The differentiator:** every identity action (login, token issue, MFA change,
revocation) is written into the tamper-evident **Grimoire** hash chain — a stock
IdP's event log is an editable database table; SHABD's is cryptographically
verifiable and cross-org anchorable via the Notary.

> **Honesty:** the built-in identity server (Praman) is POC-grade — custom
> auth/crypto needs an independent security review + pen-test before it becomes
> the auth of record for real money. Until then, prefer HS256 (internal) or point
> at a certified Keycloak. We claim *smaller attack surface, air-gap-native,
> tamper-evident audit* — not "more secure than Keycloak". See
> [`SECURITY.md`](SECURITY.md) and [`docs/PRODUCTION-READINESS.md`](docs/PRODUCTION-READINESS.md).
